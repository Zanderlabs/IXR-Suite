    ...
    def update(self):
        try:
          ...
          #  power_metrics[2] = head_movement
            ...
            DataFilter.perform_highpass(data_plot[channel], BoardShim.get_sampling_rate(self.board_id), 1.0, 4, FilterTypes.BUTTERWORTH.value, 0)
            DataFilter.perform_lowpass(data_plot[channel], BoardShim.get_sampling_rate(self.board_id), 40.0, 5, FilterTypes.CHEBYSHEV_TYPE_1.value, 1)
            ...
            for count, channel in enumerate(self.eeg_channels):
                ...
                if data.shape[1] > self.psd_size:
                    # compute psd
                    psd_data = DataFilter.get_psd_welch(data[channel], self.psd_size, self.psd_size // 2, self.sampling_rate, WindowFunctions.BLACKMAN_HARRIS.value)
          ...
          #  print('mean: ' + str(np.mean(self.inverse_workload_calib)))
           # print('std: ' + str(np.std(self.inverse_workload_calib)))
            ...
            power_metrics = self.engagement + (1-head_movement)*self.head_impact
          ...
          #  power_metrics[1] = self.relaxation.predict(feature_vector)
            ...
            self.power_bar.setOpts(height=power_metrics)
...
def main():
    ...
    try:
        ...
        Graph(board_shim,calib_length, power_length, scale, offset, head_impact)
